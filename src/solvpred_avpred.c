/* SOLVPRED 1.0 - Neural Network Prediction of Residue Solvation */
...
predict(int argc, char **argv)
{
	...
	for (winpos = 0; winpos < seqlen; winpos++)
	{
	    ...
	    avout[winpos] += activation[TOTAL - NUM_OUT];
	}
    }
    ...
    for (winpos = 0; winpos < seqlen; winpos++)
	printf("%4d %c %6.3f\n", winpos + 1, seq[winpos], avout[winpos]);
}
...
main(int argc, char **argv)
{
    ...
    /* malloc_debug(3); */
    if (argc < 2)
	fail("usage : solvpred mtx-file weight-file1 ... weight-filen");
    ifp = fopen(argv[1], "r");
    if (!ifp)
	exit(1);
    seqlen = getmtx(ifp);
